    ...
    <script type="text/javascript">
        $(document).ready(function () {
			...
			var up = $('#upload').Huploadify({
				...
				fileTypeExts:"<%=ext %>",
                formData: {'break':1, appName:"<%=appName %>",appId:"<%=appId %>",folder:"<%=folder %>",'ASPSESSID': ASPSESSID, 'AUTHID': auth},
				multi:<%=multi %>,
                ...
                onUploadError:function(file,ret){
                    alert("上传文件["+file.name+"]时，出现异常："+ret);
                }	
			});
		});
