...
<head>
    ...
    <style type="text/css">
        .hide { display: none; }
        .linkshows { height:36px;line-height:36px;width: 100%;padding-left: 10px;font-size: 14px;border-radius: 3px;border: solid 1px #999;background: #e3e3e3;}
        .linkshows a { margin-right:15px;} 
...
<script type="text/javascript">
    ...
    $(function () {
        if (window.location.href.indexOf("hideclose") > -1) {
            $("#btnClose").hide();
        }
        $("input[data-type='float'][precision='4']").after("<span class='mspan'>元</span>").parent().addClass("posrel");
        $(".RequiredStar").each(function (i, v) {
            if ($(v).text() == "*") {
                $(v).text("");
            }
        });
        $("input,textarea").addClass("Read").attr("readonly", "readonly");
        $("select,.RadioWraper,.CheckWraper").addClass("Read").attr("readonly", "readonly").attr("disabled", "disabled");
                        ...
                        switch (arrQuery[1]) {
                                    ...
                                    if (GPTAG == "1") { //基金管理机构
                                        ...
                                        switch (TAG) {
                                            case "10": //内部机构
                                                ...
                                                break;
                                            case "11": //储备项目
                                                htmlbox.push("<a openlayer='<%=base.ResolveUrl("~")%>SysFolder/Extension/BizFrame.aspx?funCode=CBXMCK&CorrelationCode=" + K_AUTOID + "&CorpID=" + vali + "' href='javascript:void(0)'>" +
                                                    CORPNAME + "</a>");
                                                break;
                                            case "12": //外部机构
                            ...
                            case "FOFCode":
                                ...
                                if (dt.Rows.length > 0) {
                                    ...
                                    htmlbox.push("<a openlayer='<%=base.ResolveUrl("~")%>SysFolder/Extension/BizFrame.aspx?funCode=JJKCK&CorrelationCode=" + K_AUTOID + "&FundID=" + vali + "' href='javascript:void(0)'>" +
                                        FUNDNAME + "</a>");
                                }
                                break;
                            case "PROJECTCODE":
