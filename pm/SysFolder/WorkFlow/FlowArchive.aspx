<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FlowArchive.aspx.cs" Inherits="EZ.WebBase.SysFolder.WorkFlow.FlowArchive" %>
...
<!--
    var _curClass = EZ.WebBase.SysFolder.WorkFlow.FlowArchive;
    ...
    $("#flex1").flexigrid
			(
			{
			    url: '../../getdata.ashx?ds=flow_archive',
			    params: [{ name: "queryid", value: "flowarchive" }
				...
				{ display: '发起公司', name: 'CompanyName', width: 100, sortable: true, align: 'center' },
    ...
    function transname(fldval, row) {
        ...
        var htmlArr = [];
        htmlArr.push("<a class='normal' href=\"javascript:dealTask('" + instanceId + "');\">" + fldval + "</a>&nbsp;");
        return htmlArr.join("");
    }
    function transfld(fldval, row) {
        var htmlArr = [];

        htmlArr.push("<a class='normal' href=\"javascript:archiveTask('" + fldval + "');\">归档</a>&nbsp;&nbsp;");
    ...
    function getgd(fldval, row) {
        if (fldval != "")
            return "<span style='color:green'>已归档</span>";
        else
            return "<span style='color:red'>未归档</span>";
    }
    ...
    function archiveTask(insId) {
        window.open("../../doc/dasaveframe.aspx?instanceid=" + insId, "_blank");
    }
