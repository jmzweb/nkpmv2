<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="IdeaTemplate.aspx.cs" Inherits="EZ.WebBase.SysFolder.WorkFlow.WfIdeaTemplate" ValidateRequest="false" %>
...
<head runat="server">
    ...
    .txtorder{width:40px;text-align:center;}
    </style>
    ...
    function AddRow(n)
    {
        for(var i=0;i<n;i++)
        {
            ...
            $("td:eq(0)", newEle).append("<input class='textbox txtname'  type=\"text\" size=\"50\" oindex=" + rowcount + " name=\"txtname\">");
            $("td:eq(1)", newEle).append("<input class='textbox txtorder' type=\"text\" size=\"50\" oindex=" + rowcount + " name=\"txtorder\" value='" + (rowcount+1) + "'>");
            $("td:eq(2)", newEle).append("<a href='javascript:' onclick=\"dropfield('" + rowcount + "');\">删除</a>");
            newEle.attr("align", "center");
            $("#tblfield").append(newEle);

            fldmodel.append("<td oindex='" + rowcount + "'  txtname='' txtorder='" + (rowcount+1) + "' state='add'/>");
