/* uiZjs v2.0.0 - http://www.uizjs.cn - uizjs.cn/license 
...
zjs.pointCutPoints = function (px, py, $div) {
    ...
    var points = [];//上右下左
    points.push({ x: centerl, y: top, julu: Math.abs(centerl - px) + Math.abs(top - py), position: 'top' });//上
    points.push({ x: right, y: centert, julu: Math.abs(right - px) + Math.abs(centert - py), position: 'right' });//右
    points.push({ x: centerl, y: bottom, julu: Math.abs(centerl - px) + Math.abs(bottom - py), position: 'bottom' });//下
    points.push({ x: left, y: centert, julu: Math.abs(left - px) + Math.abs(centert - py), position: 'left' });//左
    points.sort(function (a, b) {
        //默认按照距离排序
        return parseFloat(a.julu) > parseFloat(b.julu) ? 1 : -1
    });

    return { x: points[0].x, y: points[0].y, position: points[0].position };
};
            ...
            if (ceditorDialogC.content.document.getElementById("isclose")) {//如果是确认关闭的 而不是直接关闭 
                if (ceditorDialogC.content.document.getElementById("isclose").value) {//如果是确认关闭的 而不是直接关闭
                    try {
                        ...
                        if ($t.attr("okrefresh")) {//如果是跳转 就直接跳转
                    ...
                    if (datai[tablename]) {
                        ...
                        if (dataitem.length > 0) {
                            ...
                            $(v).find("tbody.item").each(function (ii, vv) {//再循环HTML 绑定历史数据
                                ...
                                $(vv).find("img[id]").each(function (iii, vvv) {//具体一条记录的每一个字段
                                    var $vvv = $(vvv);
                                    $vvv.attr("src", item[$vvv.attr("id")]).trigger("change");
                                });
        ...
        _event: function () {
            ...
            $id.bind("change", function () {//如果被赋值 就同步变更UL
                ...
                if (ival) {
                    ...
                    if (true) {
                        var $del = $("<a class='uploadImage_del'>删除</a>");
                        $del.click(function () {
                            fileclear();
                            $id.val("");
                            $id.trigger("change");
...
$(function () {//dom加载完成后 初始化表单 表格等
    ...
    $("input.cselectorIconFont").live("dblclick", function () {
        ...
        var dial = {
            ...
            title: "图标选择器",
            content: 'url:' + zjs.rootdir + 'iconfont.html?t=' + new Date(),
            close: function () {
                if (ceditorDialogC.content.document.getElementById('url').value) {
                    zjs.topapilock();
