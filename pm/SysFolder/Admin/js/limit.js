...
function setcheckbox(xmlnode) {
    ...
    for (var i = 0; i < soncount; i++) {
        ...
        curlist[8].checked = (limit.substr(7, 1) == "1");
        setcheckbox(sunnode);
...
function chkext(objwbs, prefix) {
    var cname = document.getElementById(prefix + objwbs).name;
    objwbs = cname.substr(4);
    var curnode = LimitData.find("tn[id='" + objwbs + "']");
    if (curnode) //如果找到
    {
        var curlist = document.getElementsByName(cname);
        var arr = [];
        for (var i = 0; i < curlist.length; i++) {
            if (curlist[i].checked)
                arr.push(curlist[i].id.substr(4));

        }

        curnode.attr("limit2", arr.join(";"));
        curnode.attr("EditFlag", "1");   
