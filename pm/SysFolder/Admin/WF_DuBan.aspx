...
<!--
	...
	({
		...
		params: [{ name: "queryid", value: "flow_admin_ondeal" }
			, { name: "condition", value: "CompanyId='[!CompanyId!]'" }
			],
		colModel: [
		...
		{ display: '停留时长', name: 'sHours', width: 60, sortable: false, align: 'left', renderer: fnTime },
		{ display: '催办', name: 'InstanceId', width: 50, sortable: false, align: 'center', renderer: transfld, hideExport: true },
		{ display: '发起人', name: 'EmployeeName', width: 40, sortable: true, align: 'left' },
		{ display: '发起部门', name: 'DeptName', width: 60, sortable: false, align: 'center' },
		...
		{ display: '公司名称', name: 'CompanyName', width: 60, sortable: true, align: 'left',hide:true },
    ...
    function fnTitle(v, row) {
        ...
        return ("<a class='normal' title='"+v+"' href=\"javascript:dealTask('" + insId + "');\" >" + v + "</a>");
        
    }
	function transfld(fldval,row)
	{
		var arr = [];
		arr.push("<a class='tdbtn'  style='background-color:#f08080;color:white;' href=\"javascript:duTask('" + fldval + "');\">&nbsp;催 办&nbsp;</a>");
		return arr.join("");
	}
