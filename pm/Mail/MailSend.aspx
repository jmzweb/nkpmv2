...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript" src="../grid/flexigrid.js?v=2016"></script>
    ...
    <style type="text/css">
	    ...
	    .flexigrid a.linkfetch{color:Red;}
	    .flexigrid a.linkfetch:hover{color:#ff1493;background-color:transparent;}
    </style>
</head>
...
<!--
    var _curClass = EIS.Web.Mail.MailSend;
    $("#flex1").flexigrid
	({
	    url: '../getdata.ashx?ds=mail',
	    restore: '<%=Request["restore"] %>',
		params: [{ name: "queryid", value: "mail_send" }
			, { name: "condition", value: "" }
			],
		colModel: [
		{ display: '选择', name: 'mailid', width: 30, sortable: false, align: 'center', renderer: tranchk },
		{ display: '主题', name: 'subject', width: 420, sortable: true, align: 'left', renderer: rendSubject },
		{ display: '收件人', name: 'receivers', width: 160, sortable: true, align: 'left' },
		{ display: '操作', name: 'mailid', width: 60, sortable: true, hide:true, align: 'center', renderer: rnFetch },
		{ display: '发送时间', name: 'createtime', width: 120, sortable: true, align: 'center' }
		],
		buttons: [
    ...
    function rnFetch(mailId) {
        return "<a href='javascript:fnFetch(&quot;" + mailId + "&quot;);' class='linkfetch' title='点击撤回邮件'>【撤回】</a>";
    }
    function fnFetch(mailId) {
        if (!confirm("您确认撤回选中的邮件吗"))
            return;

        var ret = _curClass.FetchMail(mailId,true);
        if (ret.error) {
            alert("撤回邮件时出错：" + ret.error.Message);
        }
        else {
            alert("邮件已经撤回，已经放到草稿箱");
            $("#flex1").flexReload();
        }
    }
    ...
    function app_remove(cmd, grid) {
        ...
        if (!confirm("您确认删除选中的邮件吗"))
            return;
        var ret = _curClass.RemoveMail(arr);
        if (ret.error) {
    ...
    function app_delete(cmd, grid) {
        ...
        if (!confirm("您确认删除选中的邮件吗"))
            return;
        var ret = _curClass.DeleteMail(arr);
        if (ret.error) {
