...
<body>
            ...
            <tbody>
                <% 
                    ...
                    for (var i = 0; i < ProInfo.Rows.Count; i++)
                    {
                        ...
                        htmlstr += "<td>" + drf["CAPITALSOURCE"] + "</td>"; //资金来源 
                        htmlstr += "<td>" + drf["CORPNAME"] + "</td><td >" + drf["CITYD"] + "</td><td class=''>" + drf["TRADEG_1_D"] + " - " + drf["TRADEG_2_D"] + "</td></tr>\r\n";//企业名称所在城市所属行业
                    }                                                                                                                       
                %>
                <%=htmlstr%>
            </tbody>
    ...
    <script type="text/javascript">
        //小计和合计模板
        var sumcol = [3, 4, 5, 6, 9, 11];//要合并哪些列的值
        var sumtemp = "<tr class='fontbold'><td></td><td colspan='{{hjcolspan}}'>{{text}}</td><td class='comdify'>{{sumcol3}}</td><td class='comdify'>{{sumcol4}}</td>" +
            "<td class='comdify'>{{sumcol5}}</td><td class='comdify'>{{sumcol6}}</td><td colspan='2'></td><td class='comdify'>{{sumcol9}}</td><td></td><td class='comdify'>{{sumcol11}}</td><td colspan='5'></td></tr>";
