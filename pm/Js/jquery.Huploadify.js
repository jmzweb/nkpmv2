...
(function ($) {
    $.fn.Huploadify = function (opts) {
        var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><table><tr><td colspan=2><span class="up_filename">${fileName}</span></td></tr><tr><td colspan=2><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div></td></tr><tr><td><span class="uploadbtn">上传</span><span class="delfilebtn">删除</span></td><td class="tdpro" align=right><span class="progressnum"><span class="uploadedsize"></span>/<span class="totalsize"></span></span>&nbsp;&nbsp;<span class="up_percent"></span></td></tr></table></div>';
        var defaults = {
        ...
        this.each(function () {
            var _this = $(this);
            //先添加上file按钮和上传列表
            var insNo = $('.uploadify').length + 1;
            var inputStr = '<input id="select_btn_' + insNo + '" class="selectbtn" style="display:none;" type="file" name="fileselect[]"';
            inputStr += option.multi ? ' multiple' : '';
            inputStr += ' accept="';
            inputStr += getAcceptString(option.fileTypeExts);
            inputStr += '"/>';
            inputStr += '<a id="file_upload_' + insNo + '-button" href="javascript:void(0)" class="uploadify-button">';
            inputStr += option.buttonText;
            inputStr += '</a>';
            var uploadFileListStr = '<div id="file_upload_' + insNo + '-queue" class="uploadify-queue"></div>';
            ...
            fileObj = {
                ...
                funSelect: function (files) {
                    for (var i = 0, len = files.length; i < len; i++) {
                        var file = files[i];
                        //处理模板中使用的变量
                        var $html = $(option.itemTemplate.replace(/\${fileID}/g, 'fileupload_' + insNo + '_' + file.index)
                        ...
                        if (option.showUploadedSize) {
                            $html.find('.uploadedsize').html(initFileSize);
                            $html.find('.totalsize').html(formatFileSize(file.size));
                        }

                        //判断是否显示上传百分比	
                        if (option.showUploadedPercent) {
                            $html.find('.up_percent').html(initUppercent);
                        }
                ...
                onProgress: function (file, loaded, total) {
                    var thisfile = _this.find('#fileupload_' + insNo + '_' + file.index);
                    var eleProgress = _this.find('#fileupload_' + insNo + '_' + file.index + ' .uploadify-progress');
                    ...
                    var uploadSize = parseInt(percent / 100 * total);

                    if (option.showUploadedSize) {
                        thisfile.find('.uploadedsize').text(formatFileSize(uploadSize));
                        thisfile.find('.totalsize').text(formatFileSize(total));
                    }
                    if (option.showUploadedPercent) {
                        thisfile.find('.up_percent').text(percentText);
                    }
                ...
                funGetProgressWidth: function (index) {
                    var eleProgressBar = _this.find('#fileupload_' + insNo + '_' + index + ' .uploadify-progress-bar');
                ...
                funDealFiles: function (files) {
                    ...
                    for (var i = 0, len = files.length; i < len; i++) {
                        ...
                        files[i].id = 'fileupload_' + insNo + '_' + files[i].index;
                    }
                    //执行选择回调
                ...
                funDeleteFile: function (index) {
                    for (var i = 0, len = this.fileFilter.length; i < len; i++) {
                        ...
                        if (file.index == index) {
                            ...
                            _this.find('#fileupload_' + insNo + '_' + index).fadeOut();
                            ...
                            if (xhr.readyState == 4) {
                                ...
                                if (xhr.status == 200) {
                                    ...
                                    if (option.breakPoints) {
                                        ...
                                        if (uploadedSize < fileSize) {
                                            ...
                                            if (fileObj.uploadAllowed) {
                                                ...
                                                option.formData.fileSize = originalFile.size;
                                    ...
                                    if (fileObj.uploadOver) {
                                        ...
                                        //在指定的间隔时间后删掉进度条
                                        setTimeout(function () {
                                            _this.find('#fileupload_' + insNo + '_' + originalFile.index).fadeOut();
