...
$(function () {
    ...
    jQuery(".FrameList").live("dblclick", function () {
        _clickFrame(event.srcElement);
    });
...
function _replacePara(source,subOdr,rowNum) {
    ...
    if (matches != null) {
        for (var i = 0; i < matches.length; i++) {
            ...
            if (fldName.substr(0, 1) == "."){ 
                fldName = fldName.substr(1);
                source = source.replace(matches[i], _sys.getValue(fldName,false,_rowId));
            }
            else {
                //主表的字段值
...
function _getFieldById(ctlId) {
    var fldOdr = ctlId.substr(6);
    var fldList = _sysModel.fields;
...
function _getCtlValueByFieldName(fldName, replace, rowId) {
    ...
    if (arguments.length < 3) {
        return frameElement.lhgDG.curWin._sys.getValue(fldName);
    }
    else {
...
function _setFieldValue(fldName, val, repalce, rowId) {
    ...
    if (arguments.length < 4) {
        frameElement.lhgDG.curWin._sys.setValue(fldName, val, repalce);
    }
    else {
        var fldOrder = _getFieldOdr(fldName);
        if (fldOrder == -1)
