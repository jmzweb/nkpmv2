...
(function ($) {
    var scrollIncrement = 100; var scrollDuration = 400; var tabsContainer = null; var tabsLeftScroll = null; var tabsRightScroll = null; var panelsContainer = null; var secondTabsContainer = null; 
    
    $.fn.tabs = function (options, param) {
        if (typeof options == 'string') {
                switch (options) {
                    case 'add': return addTab(param); 
                    case 'close': return closeTab(param); 
                    case 'select': return selectTab(param);
                    case 'exists': return exists(param);
        ...
        tabsContainer.bind('_resize', function () {
            ...
            { tabsLeftScroll.hide(); tabsRightScroll.hide(); }
        });
        tabsLeftScroll.hover(function () { $(this).addClass('active'); }, function () { $(this).removeClass('active'); });
        tabsRightScroll.hover(function () { $(this).addClass('active'); }, function () { $(this).removeClass('active'); }); 
        tabsLeftScroll.bind('click', function () {
            var scrollTo = tabsContainer.scrollLeft() - scrollIncrement; if (scrollTo < scrollIncrement)
                scrollTo = 0; tabsContainer.animate({ scrollLeft: scrollTo }, scrollDuration);
        }); 
        tabsRightScroll.bind('click', function () {
            var scrollTo = tabsContainer.scrollLeft() + scrollIncrement; if (scrollTo + scrollIncrement > tabsContainer.attr('scrollWidth'))
        ...
        tabsContainer.delegate('div > a.tab', 'mousedown', function (event) {
            if (!window.addEventListener && event.button == 4 || window.addEventListener && event.button == 1) { if ($(this).attr('closable') == "true") { closeTab($(this).attr('index')); } } 
        }); 
        tabsContainer.delegate('div > a.tab', 'dblclick', function () {
            if ($(this).attr('closable') == "true")
                closeTab($(this).attr('index'));
        }); 
        tabsContainer.delegate('div > a.close', 'click', function () { closeTab($(this).attr('index')); });
    }; 
    function addTab(param) {
        ...
        if (param.selected) selectTab(param.id);
    }
    function closeTab(id) {
        var iframe = window.frames['tabs_' + id + '_iframe']; try {
    ...
    function selectTab(id) {
        ...
        var $selected = $('.tabs-panel.selected', panelsContainer).removeClass('selected fadeInDown');
