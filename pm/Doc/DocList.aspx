...
<!--
        ...
        $("#flex1").flexigrid
			(
			{
				...
				//{ display: '版本号', name: 'filever', width: 80, sortable: false, align: 'center', renderer: tranver},
				...
				{ name: '新建', bclass: 'newfolder', onpress: addFolder, hidden: (limit>6 ? 0 : 1) },
				{ name: '上传', bclass: 'newfile', onpress: upLoadFile ,hidden: (limit>3 ? 0 : 1)},
				//{ name: '上传大文件', bclass: 'newfile', onpress: upLoadBig ,hidden: (limit>3 ? 0 : 1)},
				{ name: '查询', bclass: 'view', onpress: app_query },
                { separator: true },
				{ name: '属性', bclass: 'folderedit', onpress: app_FolderEdit, hidden: ("<%=folderId %>".length <36 ? 1 : (limit == 9 ? 0 : 1)) },
				{ name: '共享', bclass: 'limit', onpress: app_FolderLimit, hidden: ("<%=folderId %>" == "" ? 1 : 0) },
			    { name: '转发', bclass: 'move', onpress: app_send },
                { separator: true },
			    { name: '剪切', bclass: 'cut', onpress: app_select , hidden: (limit>3 ? 0 : 1)},
			    { name: '复制', bclass: 'copy', onpress: app_select , hidden: (limit>3 ? 0 : 1)},
			    { name: '粘贴', bclass: 'paste', onpress: app_paste , hidden:('<%=PasteHide %>'=='0' && limit>3) ? 0 : 1 },
				{ name: '删除', bclass: 'delete', onpress: app_delete },
				{ separator: true },
				{ name: '<%=fullPath %>', bclass: 'tip' }
				],
			    searchitems: [
        ...
        function tranver(v,row){
            ...
            if(ftype == "") return "";
            var arr = [];
            arr.push("<a class='ver' title='点击查看历史版本' href=\"javascript:viewHis('"+fCode+"');\">"+v+"</a>");
            if(flimit>3)
                arr.push("&nbsp;&nbsp;<a class='ner' title='上传新版本' href=\"javascript:upLoadNewVer('"+fCode+"');\">[ + ]</a>");
            return arr.join("");
        }
        ...
        function viewHis(fileCode)
        {
            openCenter("DocHistory.aspx?fileCode="+fileCode, "_blank", 800, 460);        
        }
        ...
        function upLoadBig() {
            ...
            var dlg = new $.dialog({ title: '上传大文件', maxBtn: false, page: '../SysFolder/Common/FileListH5.aspx?AppId=<%=folderId %>&folderId=<%=folderId %>'
                , btnBar: true, cover: true,  top: 50, left: 50, width: 600, height: 360, bgcolor: 'gray', cancelBtnTxt: '关闭',
                onCancel: function () {
                    $("#flex1").flexReload();
                }
        ...
        var curNode;
        function addFolder() {
            openCenter("DocFolderEdit.aspx?folderPId=<%=folderId %>", "_blank", 680, 400);
        }
        ...
        var menuFolder = [[
            ...
            { text: "属性",
                func: function () {
                    openCenter("DocFolderEdit.aspx?folderId=" + trId, "_blank", 680, 400);
                }
            }
        ]];
        ...
        var menuFile = [{
            ...
            func: function () {
                ...
                func: function () {
                    openCenter("DocFileEdit.aspx?fileId=" + trId, "_blank", 640, 400);
           }}
        ];
