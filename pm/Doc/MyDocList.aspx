<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MyDocList.aspx.cs" Inherits="EIS.Web.Doc.MyDocList" %>
...
<!--
        ...
        var _curClass = EIS.Web.Doc.MyDocList;
        var maiheight = document.documentElement.clientHeight;
        var gridData;
        $("#flex1").flexigrid
			(
			{
			    url: '../getdata.ashx?ds=doc',
			    ...
			    buttons: [
				{ name: '返回上级', bclass: 'folderback', onpress: upFolder, hidden: ("<%=folderPId %>"==""?1:0) },
				{ name: '新建文件夹', bclass: 'newfolder', onpress: addFolder },
				{ name: '上传文件', bclass: 'newfile', onpress: upLoadFile },
				{ name: '属性', bclass: 'folderedit', onpress: app_FolderEdit, hidden: ("<%=folderPId %>" == "" ? 1 : 0) },
				{ name: '共享', bclass: 'limit', onpress: app_FolderLimit, hidden: ("<%=folderPId %>" == "" ? 1 : 0) },
                { separator: true },
        ...
        function app_FolderEdit(cmd, grid) {
            openCenter("MyFolderEdit.aspx?folderId=<%=folderId %>", "_blank", 400, 300);
        }
        ...
        var curNode;
        function addFolder() {
            openCenter("MyFolderEdit.aspx?folderPId=<%=folderId %>", "_blank", 600, 300);
        }
        ...
        function readFile(fileId, _fileExt) {
            if (jQuery.inArray(_fileExt, officeExts) > -1) {
                ...
                openCenter("../SysFolder/common/filedown.aspx?para=" + para, "_self", 800, 600);
            }
        }
        ...
        var menuFolder = [[{
            ...
            , { text: "属性",
                func: function () {
                   openCenter("MyFolderEdit.aspx?folderId=" + trId, "_blank", 400, 300);
                } 
            }
        ]];
            var menuFile = [[{
            ...
            , { text: "属性",
                func: function () {
                    openCenter("MyFileEdit.aspx?fileId=" + trId, "_blank", 600, 300);
                }
            }
        ]];
        var trId = "";
